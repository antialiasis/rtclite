...
import re, socket, traceback, uuid, logging
...
class Message(object):
    ...
    def _parse(self, value):
        ...
        for h in hlist:
            try:
                ...
                logger.debug('error parsing %r', h)
                continue
    ...
    def createRequest(method, uri, headers=None, content=None):
        ...
        #logger.debug('createRequest returned\n%r', m)
        return m
...
class Stack(object):
    ...
    @property
    def newCallId(self):
        # contributed by juanantonio.ram@gmail.com, p2p-sip/r76
        # replace random.randint(0, 2**31) by uuid.uuid1()
    ...
    def _fixNatContact(self, m, src):
        if m['Contact']:
            ...
            not isLocal(src[0]) and not isLocal(uri.host) and isPrivate(uri.host) and not isPrivate(src[0]):
                logger.debug('fixing NAT -- private contact from %r', uri)
                uri.host, uri.port = src[0], src[1]
                logger.debug('to received %r', uri)
            ...
            if r.method != 'CANCEL' and 'tag' in r.To: # for existing dialog
                ...
                if not d: # no dialog found
                    if r.method != 'ACK':
                        ...
                        logger.debug('no dialog for ACK, finding transaction')
                        if not t and branch != '0': t = self.findTransaction(Transaction.createId(branch, 'INVITE'))
                        if t and t.state != 'terminated':
                            logger.debug('Found transaction %r', t)
                            t.receivedRequest(r)
                            return
                        else:
                            logger.debug('No existing transaction for ACK')
                            u = self.createServer(r, uri)
                            if u: app = u
                            else:
                                logger.debug('Ignoring ACK without transaction')
                                return
                else: # dialog found
                    app = d
    ...
    def _receivedResponse(self, r, uri):
        ...
        if not t:
            if method == 'INVITE' and r.is2xx: # success of INVITE
                ...
                logger.debug('transaction id %r not found', Transaction.createId(branch, method)) # do not print the full transactions table
...
class Transaction(object):
    ...
    def close(self):
        ...
        self.stopTimers()
        if self.stack:
            logger.debug('closing transaction %r', self.id)
...
class InviteServerTransaction(Transaction):
    ...
    def receivedRequest(self, request):
        if self.request.method == request.method: # retransmitted
            if self.state == 'proceeding' or self.state == 'completed':
                ...
                logger.debug('Retransmitting (#%d) INVITE[%s] response due to retransmission from remote endpoint', self.retrans, self.id)
                self.stack.send(self.lastResponse, self.remote, self.transport)
    ...
    def timeout(self, name, timeout):
        if self.state == 'completed':
            if name == 'G':
                ...
                self.retrans = self.retrans + 1
                logger.debug('Retransmitting (#%d) INVITE[%s] response', self.retrans, self.id)
                self.stack.send(self.lastResponse, self.remote, self.transport)
            elif name == 'H':
                self.state = 'terminated'
                self.app.timeout(self)
    ...
    def sendResponse(self, response):
        ...
        if response.is1xx:
            if self.state == 'proceeding' or self.state == 'trying':
                self.stack.send(response, self.remote, self.transport)
        # not using - contributed by juanantonio.ram@gmail.com, p2p-sip/r75
        # comment out the following elif block, so that 2xx and failure have same processing.
        elif response.is2xx:
            if self.state == 'proceeding' or self.state == 'trying':
                self.state = 'terminated'
                self.stack.send(response, self.remote, self.transport)
...
class UserAgent(object):
        ...
        if request.Route:
            ...
            if len(routes) > 0:
                ...
                if not target or 'lr' not in target.param: # strict route
                    logger.debug('strict route target=%r routes=%r', target, routes)
                    del routes[0] # ignore first route
                    if len(routes) > 0:
                        logger.debug('appending our route')
    ...
    def timeout(self, transaction):
        ...
        if transaction and transaction != self.transaction: # invalid transaction
            logger.debug('invalid transaction in timeout() %r != %r', transaction, self.transaction)
            return
...
class Dialog(UserAgent):
    ...
    def createServer(stack, request, response, transaction):
        ...
        while d.routeSet and isMulticast(d.routeSet[0].value.uri.host): # remove any multicast address from top of the list.
            logger.debug('deleting top multicast routeSet %r', d.routeSet[0])
            del d.routeSet[0]
            if len(d.routeSet) == 0: d.routeSet = None
        d.secure = request.uri.secure
        # contributed by juanantonio.ram@gmail.com, p2p-sip/r75
        # bug fix - replace d.localSeq to d.remoteSeq.
        ...
        logger.debug('request contact %r', request.Contact)
    ...
    def sendCancel(self):
        ...
        if len(self.clients) == 0:
            logger.debug('No client transaction to send cancel')
            return
    ...
    def receivedRequest(self, transaction, request):
        ...
        if self.remoteSeq != 0 and request.CSeq.number < self.remoteSeq:
            logger.debug('Dialog.receivedRequest() CSeq is old %r < %r', request.CSeq.number, self.remoteSeq)
    ...
    def error(self, transaction, error):
        ...
        if transaction is None:
            ...
            if self.request.method != 'ACK':
                ...
                logger.warn('dropping ACK:', error)
        branch = self.getBranch(transaction)
        if not branch:  return # invalid transaction
        self.transaction = None
        branch.transaction = None
